...
the \code{OMP\_PROC\_BIND} environment variable or the \code{proc\_bind} clause on
a \code{parallel} construct. The specification of the binding places can be set 
in the \code{OMP\_PLACES} environment variable. 
...
\code{OMP\_PROC\_BIND} or \code{proc\_bind} will be applied to the subset of 
...
\code{OMP\_PROC\_BIND} variable or the \code{proc\_bind} clause  to 
