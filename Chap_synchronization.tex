...
The \code{barrier} construct is a stand-alone directive that forces all threads
...
The \code{critical} construct is a directive and contains a structured block. 
...
act cooperatively (only one thread at a time in any \code{critical} region),
or as separate (only one thread at a time in each \code{critical} regions when
...
\code{barrier} construct.  Also, additional synchronizations are available for tasks.
All children of a task will wait at a \code{taskwait} for their siblings
to complete.  A \code{taskgroup} construct creates a region in which the
