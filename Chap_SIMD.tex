...
data within the loop.  The \code{private}, \code{reduction} and 
\code{lastprivate} clauses provide data-sharing attributes.  Clauses 
also provide vector length preference/restrictions (\code{simdlen} 
/ \code{safelen}), loop fusion (\code{collapse}), and data 
...
execution in different SIMD units.  Hence, the \code{simd} construct can be 
