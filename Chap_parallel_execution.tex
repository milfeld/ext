...
\code{parallel} region, and joins them (removes from exection) at the 
...
%   would like to break this into sections
...
\code{collapse(\#)} clause designates more than one \plc{associated} loop
to be executed in parallel, by forming a "single iteration space" for 
the specified number of nested loops.  The \code{ordered(\#)} clause can 
...
\code{parallel} regions.  The default setting for dynamic adjustment is implemention 
...
by threads of the team.  The control part of the Fortran \code{where} and 
