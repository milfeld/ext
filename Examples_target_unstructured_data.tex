%begin 
\pagebreak
\section{\code{target} \code{enter}/\code{exit} \code{data} Constructs}
\label{sec:target_enter_exit_data}
%\section{Simple target enter data and target exit data Constructs}

The structured data construct (\code{target data}) provides persistent data on a
device for subsequent \code{target} constructs as shown in the 
\code{target data} examples above. This is accomplished by creating a single
\code{target data} region containing \code{target} constructs.
...
with the \code{target enter data} and \code{target exit data} constructs.
