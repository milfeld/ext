...
A loop can be vectorized even though the iterations are not completely independent when it has loop-carried dependences that are forward lexical dependences (indicated by the read of \plc{A[j+1]} to write \plc{A[j]}, or \plc{A(j+1)} and \plc{A(j)} in Fortran). That is, the read of \plc{A[j+1]} (or \plc{A(j+1)} in Fortran) before write to \plc{A[j]} (or \plc{A(j)} in Fortran) ordering must be preserved for each iteration \plc{j} as a valid SIMD code generation.
